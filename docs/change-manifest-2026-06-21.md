...
下面这些地方现在仍然明显带有 CSI300 或基准约束，不建议我擅自改成全市场，先留给你拍板：

- `backend/api/backtest.py`：回测基准仍是 CSI300 等权近似，约束分析也以 CSI300 为主。
- `backend/api/factors.py`：多处仍按 `csi300` 作为分析样本和缓存键。
- `backend/api/mean_reversion.py`：均值回归默认仍读取 `instruments/csi300.txt`。
- `backend/core/dl_models.py`：训练样本仍偏 CSI300。
- `backend/api/risk.py`：压力测试和事件回顾仍以 CSI300 区间收益为参照。

如果你希望，我下一步可以只做一件事中的一种：

1. 保持这些模块的 CSI300 定位，只把页面和说明写清楚。
2. 逐个模块改成“全市场 A 股模式”，但这会改动范围更大，需要你确认优先级。
